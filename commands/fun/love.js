...
aliases: [""],
  ...
  usage:
    "[mention(1) | ID(1) | name(1) | nickname(1)] [mention(2) | ID(2) | name(2) | nickname(2)]",
  execute: async (bot, message, args) => {
    let user =
      ...
      message.guild.members.cache.find(
        mp => mp.displayName.toLowerCase() === args[0].toLocaleLowerCase()
      );
    let user2 =
      (await message.mentions.members.array()[1]) ||
      message.guild.members.cache.get(args[1]) ||
      message.guild.members.cache.find(
        m => m.user.username.toLowerCase() === args[1].toLocaleLowerCase()
      ) ||
      message.guild.members.cache.find(
        mp => mp.displayName.toLowerCase() === args[1].toLocaleLowerCase()
      );
    if (!args[0]) return message.channel.send("**Enter Name Of Lover!**");
    if (!args[1])
      return message.channel.send("**Enter Name Of Another Lover!**");
