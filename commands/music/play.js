...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (!args.length) {
      ...
      embed.setAuthor("WRONG SYNTAX : Type `Name` Or `URL`");
      ...
      try {
        ...
        if (error.errors[0].domain === "usageLimits") {
          return message.channel.send(
            "Your YT API limit is over and it will be restored under 24 hours"
          );
        }
      }
    }
    ...
    if (serverQueue) {
      ...
      embed.setDescription(
        `**[${song.title}](${song.url}) - [${message.author}]**`
      );
      embed
    ...
    if (!serverQueue) {
      try {
        ...
        return message.channel
          .send({
            embed: {
              description: `Couldn't Join The Channel: ${error}`,
              color: "#00FFFF"
            }
          })
          .catch(console.error);
