...
module.exports = {
  ...
  execute: async (client, message, args) => {
    ...
    const data = await fetch(
      `https://some-random-api.ml/pokedex?pokemon=${encodeURI(args.join(" "))}`
    )
      .then(res => res.json())
      .catch(() => null);
    ...
    let errmsg = new MessageEmbed()
      .setDescription(
        `\u200b\n${message.author},  Pokédex is currently down!\n\u200b`
      )
      .setColor("RED");

    if (!data)
      return (await prompt.edit(errmsg).catch(() => null))
        ? null
        : message.channel.send(errmsg).catch(() => null);

    errmsg.setDescription(
      `\u200b\n${message.author},  I can't seem to find **${args.join(
        " "
      )}** from the Pokédex!\n\u200b`
    );
    ...
    const { hp, attack, defense, sp_atk, sp_def, speed } = stats
      ? stats
      : {
          hp: null,
          attack: null,
    ...
    const { evolutionLine, evolutionStage } = family
      ? family
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        "Egg Groups",
        `\u200B${
          egg_groups && egg_groups.length ? egg_groups.join("\n") : "\u200B"
        }`,
        true
      )
      ...
      .addField(
        "Evolution Line",
        evolutionLine.length ? evolutionLine.join(" -> ") : `\u200B`,
        true
      )
